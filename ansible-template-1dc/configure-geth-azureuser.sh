...
if [ $NODE_TYPE -eq 0 ]; then # TX nodes only
	...
	wget -N ${ARTIFACTS_URL_PREFIX}main.handlebars || unsuccessful_exit "failed to download main.handlebars" 18;
	cd $ETHERADMIN_HOME/views;
	wget -N ${ARTIFACTS_URL_PREFIX}etheradmin.handlebars || unsuccessful_exit "failed to download etheradmin.handlebars" 19;
	wget -N ${ARTIFACTS_URL_PREFIX}etherstartup.handlebars || unsuccessful_exit "failed to download etherstartup.handlebars" 20;
	cd $ETHERADMIN_HOME;
	wget -N ${ARTIFACTS_URL_PREFIX}package.json || unsuccessful_exit "failed to download package.json" 21;
	wget -N ${ARTIFACTS_URL_PREFIX}npm-shrinkwrap.json || unsuccessful_exit "failed to download npm-shrinkwrap.json" 22;
	npm install || unsuccessful_exit "failed while running npm install" 23;
	wget -N ${ARTIFACTS_URL_PREFIX}app.js || unsuccessful_exit "failed to download app.js" 24;
	mkdir $ETHERADMIN_HOME/public;
	cd $ETHERADMIN_HOME/public;
	wget -N ${ARTIFACTS_URL_PREFIX}skeleton.css || unsuccessful_exit "failed to download skeleton.css" 25;
